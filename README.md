...
scala> val textMatchers = Seq("glob:b?t", "glob:b*t", "glob:*.log", "regex:b+", "literal:b*t", "glob:id_{dsa,rsa}").map(TextMatcher(_))
...
scala> Seq("bit", "bat", "bot", "boot", "b*t", "id_dsa", "id_rsa", "app.log").map(s => s -> textMatchers.filter(_.r.matches(s)).mkString(" ")).toMap
